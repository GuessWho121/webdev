document.addEventListener("DOMContentLoaded", () => {
    ...
    document.querySelector("form").addEventListener("submit", async function (event) {
      ...
      try {
        // Submit recipient form data
        const response = await fetch("http://127.0.0.1:8000/submit-recipient-form", {
          method: "POST",
          headers: {
        ...
        if (!response.ok) {
          throw new Error(data.detail || "Failed to submit recipient form")
        }
  
        alert("Recipient form submitted successfully!")
